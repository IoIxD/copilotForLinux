...
void setup_loading(widgets_ctx *ctx) {
  ...
  ctx->image = gtk_image_new_from_pixbuf(pixbuf);

  ctx->loading_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  gtk_box_set_spacing(GTK_BOX(ctx->loading_box), 10);
  gtk_box_prepend(GTK_BOX(ctx->loading_box), ctx->image);
  gtk_widget_set_hexpand(GTK_WIDGET(ctx->image), true);
  gtk_widget_set_vexpand(GTK_WIDGET(ctx->image), true);

  gtk_box_append(GTK_BOX(ctx->main_box), GTK_WIDGET(ctx->loading_box));
}

void loading(WebKitWebView *webview, WebKitLoadEvent load_event,
             gpointer user_data) {
  widgets_ctx *ctx = (widgets_ctx *)user_data;
  assert(ctx);
  assert(ctx->webView);

  bool isLoading = webkit_web_view_is_loading(ctx->webView);
  if (isLoading && !ctx->alreadyLoaded) {
    printf("loading box\n");
    gtk_widget_set_visible(GTK_WIDGET(ctx->loading_box), true);
    gtk_widget_set_visible(GTK_WIDGET(ctx->webView), false);
  } else {
    printf("webview\n");
    gtk_widget_set_visible(GTK_WIDGET(ctx->loading_box), false);
    gtk_widget_set_visible(GTK_WIDGET(ctx->webView), true);
    ctx->alreadyLoaded = true;
  }
